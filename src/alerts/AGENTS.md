...
relevant to Crescent City, CA. 15 independent monitors (8 core + 7 extended)
feed a composite severity scoring system and unified alert analytics timeline.
...
- **Composite severity**: `severity.ts` aggregates all 15 monitors (8 core + 7 extended) into CALM → EMERGENCY; `composite.ts` shapes the per-monitor inputs + classifies source health so the runner stays thin.
...
bun run alerts              # all 15 concurrently + composite severity (scripts/run-alerts.ts)
