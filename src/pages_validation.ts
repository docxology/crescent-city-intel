/**
 * Generated public Pages artifact validator — release-gate checks for the
 * bounded static snapshot (required assets, SEO/JSON-LD, a11y, caching,
...
export async function validatePagesArtifact(destination: string): Promise<string[]> {
  ...
  if (snapshot) {
    ...
    if (snapshot.files?.codeSearchIndex) {
      ...
      for (const [fileKey, kind, prefix, budget] of [
        ...
        const shardPath = (snapshot.files as unknown as Record<string, string | null | undefined>)?.[fileKey];
  ...
  const staticPagesDir = resolve(import.meta.dir, "pages/static");
